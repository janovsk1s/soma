...
- Optional cloud photo analysis uses a correctly oriented temporary JPEG bounded
  ...
  provider latency, and peak memory. When a photo request is rate-limited and
  the provider names a short wait, the explicit user-triggered proposal pauses
  exactly that long and retries once at the same full quality — the image is
  never downscaled, because a degraded proposal is worse than an honest
  failure. Ambient features never retry.
